DynamicSitemaps::Sitemap.draw do
  ...
  new_page!
  DataProvider::Scheme.active.each do |scheme|
    url mutual_fund_url(scheme), :change_freq => 'daily', :priority => '0.64'

    url main_app.scheme_returns_mutual_fund_url(scheme), :change_freq => 'daily', :priority => '0.64'
    url main_app.top_holdings_mutual_fund_url(scheme), :change_freq => 'daily', :priority => '0.64'
    url main_app.detailed_holdings_mutual_fund_url(scheme), :change_freq => 'daily', :priority => '0.64'
    url main_app.asset_allocation_mutual_fund_url(scheme), :change_freq => 'daily', :priority => '0.64'
    url main_app.sectoral_allocation_mutual_fund_url(scheme), :change_freq => 'daily', :priority => '0.64'

  end

  # mutual funds pages
  new_page!
  DataProvider::Company.all.each do |company|
    url stock_url(company), :change_freq => 'daily', :priority => 0.64

    url main_app.stock_balance_sheet_url(company), :change_freq => 'daily', :priority => '0.64'
    url main_app.stock_profit_loss_url(company), :change_freq => 'daily', :priority => '0.64'
    url main_app.stock_cash_flow_url(company), :change_freq => 'daily', :priority => '0.64'
    url main_app.stock_ratios_url(company), :change_freq => 'daily', :priority => '0.64'
    url main_app.stock_share_holding_url(company), :change_freq => 'daily', :priority => '0.64'
  end
